// Find the greatest product of any four adjacent numbers in
// a 20x20 grid.
  ...
  for(i = 0; i < 20; i++) {
    for(j = 0; j < 20; j++) {
      ...
      if(i < 17 && j < 17) {
        ...
        if(current > max) {
          max = current;
          printf("down-right max! %d = %d, %d, %d, %d\n",
            current, grid[i][j], grid[i+1][j+1], grid[i+2][j+2], grid[i+3][j+3]);
        }
      }
      // go down-left
      if(i < 17 && j > 2) {
        ...
        if(current > max) {
          max = current;
          printf("left-right max! %d = %d, %d, %d, %d\n",
            current, grid[i][j], grid[i+1][j-1], grid[i+2][j-2], grid[i+3][j-3]);
        }
      }
    }
